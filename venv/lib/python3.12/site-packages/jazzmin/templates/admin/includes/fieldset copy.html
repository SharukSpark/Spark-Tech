...
<div class="card {{ fieldset.classes|cut:" collapse" }}">
    ...
    <div class="card-header">
        <div class="card-title">
            <strong>{{ fieldset.name }}</strong>{% if fieldset.description %} - <i>{{ fieldset.description }}</i>{%
            endif %}
        </div>
    </div>
    ...
    <div class="p-5{% if fieldset.name %} card-body{% endif %}">
        ...
        <div class="form-group{% if line.fields|length == 1 and line.errors %} errors{% endif %}">
            ...
            <div class="row">
                {% for field in line %}
                <label
                    class="{% if not line.fields|length_is:'1' and forloop.counter != 1 %}col-auto {% else %}col-sm-3 {% endif %}text-left"
